...
public class CuiPopup extends LinearLayout {

    public static final String TAG = CuiPopup.class.getName();
    ...
    public CuiPopup(Context context) {
        this(context, null);
    }

    public CuiPopup(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CuiPopup(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.cui_popup_layout);
        if (typedArray != null) {
            defaultHeight = typedArray.getDimension(R.styleable.cui_popup_layout_defaultHeight, 0);
            realHeight = typedArray.getDimension(R.styleable.cui_popup_layout_realHeight, 0);
            typedArray.recycle();
        }
        init();
    }
...
//                break;
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (android.os.Build.VERSION.SDK_INT < 14) {
                if (childView instanceof AbsListView) {
                    ...
                    noHaveScroll = true;
                    return absListView.getChildCount() > 0 && (absListView.getFirstVisiblePosition() > 0 || childView.getTop() < absListView.getPaddingTop());
                } else {
                    noHaveScroll = true;
    ...
    /**
     * titleview可以添加多个，必须是在activity布局内声明的view，且不在cui_popup_layout内部，titleview将自己消费touch事件
     */
    ...
    /**
     * touchView只能添加一个，必须是在activity布局内声明的view，且必须在cui_popup_layout内部，touchview内部的子view将自己消费touch事件
     */
    public CuiPopup addTouchView(View touchView) {
        this.touchView = touchView;
        return this;
    }

    public CuiPopup setOnScrollStateListener(OnScrollStateListener onScrollStateListener) {
